...
function createVideoTrun(
  ...
  for (const sample of samples) {
    ...
    const compositionTimeOffset = sample.compositionTimeOffset ?? 0;
    bytes[bytesOffset++] = (compositionTimeOffset & 0xff000000) >>> 24;
    bytes[bytesOffset++] = (compositionTimeOffset & 0xff0000) >>> 16;
    bytes[bytesOffset++] = (compositionTimeOffset & 0xff00) >>> 8;

    // sample_composition_time_offset
    bytes[bytesOffset++] = compositionTimeOffset & 0xff;
  }
